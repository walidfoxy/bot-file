...
def getinfobyid(packet , user_id , client):
    ...
    if "id" not in name:
            ...
            #uid
        pyload_3 = gen_msgv2_clan(packet , f"""[00FFFF][b][c]الأيدي -->>""")
        client.send(bytes.fromhex(pyload_3))
        pyload_3 = gen_msgv2(packet , f"""[00FFFF][b][c]الأيدي -->>""")
        client.send(bytes.fromhex(pyload_3))
        #uid
    ...
    #name
        ...
        client.send(bytes.fromhex(pyload_3))
        pyload_3 = gen_msgv2(packet , f"""[00FFFF][b][c]إسم الاعب -->>""")
        client.send(bytes.fromhex(pyload_3))
        #name-end
        ...
        client.send(bytes.fromhex(pyload_3))
    #stat
        pyload_3 = gen_msgv2_clan(packet , f"""[00FFFF][b][c]حالة الحساب :""")
        client.send(bytes.fromhex(pyload_3))
        pyload_3 = gen_msgv2(packet , f"""[00FFFF][b][c]حالة الحساب :""")
        client.send(bytes.fromhex(pyload_3))
        #stat
        time.sleep(4.0)
        pyload_3 = gen_msgv2_clan(packet , f"""[00FFFF][b][c] جاري تسجيل دخول حسابك...""")
        client.send(bytes.fromhex(pyload_3))
        pyload_3 = gen_msgv2(packet , f"""[00FFFF][b][c]جاري تسجيل دخول حسابك...""")
        client.send(bytes.fromhex(pyload_3))
        #5arena
        time.sleep(4.0)
        pyload_3 = gen_msgv2_clan(packet , f"""[00FFFF][b][c]الرسالة من قبل الشركة -->>""")
        client.send(bytes.fromhex(pyload_3))
        pyload_3 = gen_msgv2(packet , f"""[00FFFF][b][c]الرسالة من قبل الشركة -->>""")
        client.send(bytes.fromhex(pyload_3))
        #stat
                        ...
                        if '0300' in dataS.hex()[0:4] :
                            ...
                            if '0500' in dataS.hex()[:4] and b'\x05\x15\x00\x00\x00\x10Z\xca\xf5&T;\x0cA\x01\x16\xe0\x05\xb2\xea\xe4\x0b' in dataC:
                                ...
                                if '1200' in dataS.hex()[0:4] and '2f646573' in dataS.hex()[0:900] : 
                                    ...
                                    client.send(bytes.fromhex(gen_msgv2(dataS.hex() ,"[00FFFF][b][c]تدمير سكواد -->> [00ff00][b][c] مفعل")))
                                ...
                                if '1200' in dataS.hex()[0:4] and '3f6c766c' in dataS.hex()[0:900] :
                                    ...
                                    time.sleep(2.0)
                                    client.send(bytes.fromhex(gen_msgv2(dataS.hex() ,"[00FF00][b][c]المود : ذئب الوحيد ")))
                                    client.send(bytes.fromhex(str(gen_msgv2_clan(dataS.hex() ,"[00FF00][b][c]المود : ذئب الوحيد "))))
...
#           /2
                                ...
                                if '1200' in dataS.hex()[0:4]:
                                    if b"/ca" in dataS:
                                        ...
                                        client.send(bytes.fromhex(gen_msgv2(dataS.hex() ,"[00FF00][b][c]توقف تلقائيا !")))
                                        client.send(bytes.fromhex(str(gen_msgv2_clan(dataS.hex() ,"[00FF00][b][c]توقف تلقائيا!"))))
